...
public class JEP {
	...
	//private Node topNode; modified by lm
	public Node topNode;
	...
	/** ResourceBundle dealing with localized messages: added by lm */
	...
	/** Initialize the ResourceBundle dealing with localized messages: added by lm.
	 * To avoid inconsistencies, this behavior should be instead included in all the constructors.
	 * @param locale
	 */
	public void setMessageBundle(Locale locale) {
        JEP.msg = ResourceBundle.getBundle("datafiles/messages", locale); //$NON-NLS-1$
	}


	/** Return the message with the specified id, as read from the localized ResourceBundle: added by lm.
	 * @param id
	 * @return message
	 */
    public static String getMessage(String id) { return msg.getString(id); }
	...
	public void addStandardFunctions() {
		...
		//funTab.put("sin", new Sine()); //commented out by lm
		//funTab.put("cos", new Cosine()); //commented out by lm
		...
		//funTab.put("log", new Logarithm()); //commented out by lm
		funTab.put("ln", new NaturalLogarithm());
		funTab.put("exp", new Exp());
		funTab.put("pow", new Power());

		//funTab.put("sqrt",new SquareRoot()); //commented out by lm
		//funTab.put("abs", new Abs()); //commented out by lm
		//funTab.put("mod", new Modulus()); //commented out by lm
		//funTab.put("sum", new Sum()); //commented out by lm

		//funTab.put("rand", new org.nfunk.jep.function.Random()); //commented out by lm
		
		// rjm additions
		//funTab.put("if", new If()); //commented out by lm
		//funTab.put("str", new Str()); //commented out by lm
	...
	public void addStandardConstants() {
		...
		//symTab.addConstant("pi", new Double(Math.PI)); //commented out by lm
		//symTab.addConstant("e", new Double(Math.E)); //commented out by lm
		symTab.addConstant("pi", Tensor.newScalar(Math.PI));
		symTab.addConstant("e", Tensor.newScalar(Math.E));
	}
	...
	*/
	...
	public Object getValueAsObject() {
		...
		try { // evaluate the expression
			...
			//errorList.addElement(JEP.getMessage("JEP.ERR.EVAL") + ": " + e.getMessage()); // modified by lm
			errorList.addElement(e.getMessage()); // modified by lm
			return null;
	 ...
	 */
	public boolean hasError() { return !errorList.isEmpty(); }
	...
	public String getErrorInfo() {
		if(hasError()) {
			...
			for(int i=0; i<errorList.size(); i++) { str += errorList.elementAt(i) + "\n"; }
			return str;
		}
		return null;
	}
