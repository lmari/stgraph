...
public class VertexRenderer extends JLabel implements CellViewRenderer,
	...
	public Component getRendererComponent(JGraph graph, CellView view,
		...
		//TODO [LM, 30.5.13] What follows is a peculiar fix for a peculiar a bug: 
		// font, font color, and bounds do not update after (mouse) node resize...
		// and this because the model is correctly updated, but the view is not:
		// it is as if (mouse) node resize partly detaches the view from its model?!
		// Fixed (not solved) by explicitly implementing the related methods in STNode
		/*
		try {
			AttributeMap map = view.getAllAttributes();
			map.put("font", ((DefaultGraphCell)view.getCell()).attributes.get("font"));
			map.put("foregroundColor", ((DefaultGraphCell)view.getCell()).attributes.get("foregroundColor"));
