...
/** NodeDialog handler for the FourButton widget. */
@SuppressWarnings("serial")
public class FourButtonWidgetDialog extends WidgetDialog {
	...
	protected void fill() {
		...
		Integer i = (Integer)node.getProperty(FourButtonWidget.PROP_ACTION);
	...
	private JPanel getConfActionPanel() {
		if(confActionPanel == null) {
			...
			action1 = new JRadioButton(STGraphC.getMessage(FourButtonWidget.PREFIX + ".ACTION1")); //$NON-NLS-1$
			action2 = new JRadioButton(STGraphC.getMessage(FourButtonWidget.PREFIX + ".ACTION2")); //$NON-NLS-1$
	...
	protected void okHandler() {
		...
		if(!((FourButtonWidget)node).setProps(name, action)) { return; }
		completeExit();
	}
