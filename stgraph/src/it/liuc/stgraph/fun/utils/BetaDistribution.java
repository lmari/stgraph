...
public class BetaDistribution {
	...
	/*
	static double a_last = 0.0, b_last = 0.0;
	static double a_;
	static double b_;
	static double t;
	static double fa;
	...
	//static double p_last = 0.0, q_last = 0.0;
	//static double a, b, s, m, D, Dl, x1, x2, x4, x5, f1, f2, f4, f5;
	//static double ll, lr, z2, z4, p3, p4;
	...
	public static double pdf(double x, double alpha, double beta) {
		...
		double PDF_CONST = Fun.logGamma(alpha+beta) - Fun.logGamma(alpha) - Fun.logGamma(beta);
	...
	public static double nextDouble(double alpha, double beta) {
		...
		double b = beta;
		if (a  > 1.0) {
			if (b  > 1.0)  return(b1prs(a, b));
			if (b  < 1.0)  return(1.0 - b01(b, a));
			if (b == 1.0) {
				return(Math.exp(Math.log( Math.random()) / a));
			}
		}

		if (a  < 1.0) {
			if (b  > 1.0)  return(b01(a, b));
			if (b  < 1.0)  return(b00(a, b));
			if (b == 1.0) {
				return(Math.exp(Math.log(Math.random()) / a));
			}
		}
		...
		if (a == 1.0) {
			if (b != 1.0)  return(1.0 - Math.exp(Math.log(Math.random()) / b));
			if (b == 1.0)  return(Math.random());
		}

		return 0.0;
	}

	protected static double b00(double a, double b) {
		...
		for (;;) {
			if ((U = Math.random() * p2) <= p1) {       //  X < t  
				Z = Math.exp(Math.log(U/p1) / a);  X = t*Z;
				// squeeze accept:   L(x) = 1 + (1 - b)x                                 
				if ((V = Math.random() * fb) <= 1.0 - b_*X)  break;
				...
				if ((V = Math.random() * fa) <= 1.0 - a_*(1.0 - X))  break;
	...
	protected static double b01(double a, double b) {
		double a_last = 0.0, b_last = 0.0;
		double a_ = 0.0;
		double b_ = 0.0;
		double t = 0.0;
		...
		for (;;) {
			if ((U = Math.random() * p2) <= p1) {       //  X < t  
				...
				if ((V = Math.random() ) <= 1.0 - ml*X)  break;
	...
	protected static double b1prs(double p, double q) {
		double p_last = 0.0, q_last = 0.0;
		double a = 0.0, b = 0.0, s = 0.0, m = 0.0, D = 0.0, Dl = 0.0, x1 = 0.0, x2 = 0.0, x4 = 0.0, x5 = 0.0, f1 = 0.0, f2 = 0.0, f4 = 0.0, f5 = 0.0;
		double ll = 0.0, lr = 0.0, z2 = 0.0, z4 = 0.0, p3 = 0.0, p4 = 0.0;
