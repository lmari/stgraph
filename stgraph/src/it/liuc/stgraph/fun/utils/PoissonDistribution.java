...
public class PoissonDistribution {
	...
	static double p,q,p0;
	static double[] pp = new double[36];
	static int llll;
	// cache for >= SWITCH_MEAN
	static double my_last = -1.0;
	static double ll;
	static int k2, k4, k1, k5;
	static double dl, dr, r1, r2, r4, r5, lr, l_my, c_pm;
	static double f1, f2, f4, f5, p1, p2, p3, p4, p5, p6;
	// cache for both;
	static int m;
	static final double MEAN_MAX = Integer.MAX_VALUE; // for all means larger than that, we don't try to compute a poisson deviation, but return the mean.
	static final double SWITCH_MEAN = 10.0; // switch from method A to method B


	/**
	 * Get the probability distribution function.
	 */
	public static double pdf(final double mean, final int k) {
		return Math.exp(k * Math.log(mean) - Arithmetic.logFactorial(k) - mean);
	}
	 ...
	 */
	public static double cdf(final double mean, final int k) {
		return Probability.poisson(k, mean);
	}
	 ...
	 */
	public static int nextInt(final double mean) {
		/******************************************************************
		 *                                                                *
		...
		if(my < SWITCH_MEAN) { // CASE B: Inversion- start new table and calculate p0
			...
			m = (my > 1.0) ? (int)my : 1;
			for(;;) {
				u = Math.random();           // Step U. Uniform sample 
				k = 0;
				if(u <= p0) { return(k); }
