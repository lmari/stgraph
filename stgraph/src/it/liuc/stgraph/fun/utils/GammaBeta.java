 ...
 */
public class GammaBeta extends Constants {
	/**
	...
	static public double gamma(double x) throws ArithmeticException {
		...
		if( q > 33.0 ) {
			if( x < 0.0 ) {
				p = Math.floor(q);
				if( p == q ) throw new ArithmeticException("gamma: overflow"); //$NON-NLS-1$
				z = q - p;
				if( z > 0.5 ) {
					p += 1.0;
					z = q - p;
				...
				if( z == 0.0 ) throw new ArithmeticException("gamma: overflow"); //$NON-NLS-1$
				z = Math.abs(z);
		...
		while( x < 0.0 ) {
			if( x == 0.0 ) {
				throw new ArithmeticException("gamma: singular"); //$NON-NLS-1$
			} else
				if( x > -1.E-9 ) {
	...
	public static double incompleteBeta( double aa, double bb, double xx ) throws ArithmeticException {
		...
		ArithmeticException("ibeta: Domain error!"); //$NON-NLS-1$

		if( (xx <= 0.0) || ( xx >= 1.0) ) {
			if( xx == 0.0 ) return 0.0;
			if( xx == 1.0 ) return 1.0;
			throw new ArithmeticException("ibeta: Domain error!"); //$NON-NLS-1$
		}
	...
	public static double logGamma(double x) throws ArithmeticException {
		...
		if( x < -34.0 ) {
			...
			w = logGamma(q);
			p = Math.floor(q);
			if( p == q ) throw new ArithmeticException("lgam: Overflow"); //$NON-NLS-1$
			z = q - p;
			if( z > 0.5 ) {
				p += 1.0;
				z = p - q;
			...
			z = q * Math.sin( Math.PI * z );
			if( z == 0.0 ) throw new 
			ArithmeticException("lgamma: Overflow"); //$NON-NLS-1$
	...
	static double powerSeries( double a, double b, double x ) throws ArithmeticException {
		...
		if( (a+b) < MAXGAM && Math.abs(u) < MAXLOG ) {
			t = GammaBeta.gamma(a+b)/(GammaBeta.gamma(a)*GammaBeta.gamma(b));
			s = s * t * Math.pow(x,a);
		} else {
			t = GammaBeta.logGamma(a+b) - GammaBeta.logGamma(a) - GammaBeta.logGamma(b) + u + Math.log(s);
			if( t < MINLOG ) 	s = 0.0;
			else  	            s = Math.exp(t);
		}
		return s;
