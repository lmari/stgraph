...
/** Ekit
  ...
  *
  * @author Howard Kistler (minor changes by Luca Mari: marked with LM)
  * @version 1.5
  *
  * REQUIREMENTS
...
{
	private EkitCaller caller = null; //LM//
	...
	/** Master Constructor
	  ...
	  */
	public Ekit(final EkitCaller caller, String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage, String sCountry, boolean base64, boolean debugMode, boolean useSpellChecker, boolean multiBar, boolean enterBreak) //LM//
	{
    	this.caller = caller; //LM//

    	if(useSpellChecker)
		{
    		//LM//ekitCore = new EkitCoreSpell(false, sDocument, sStyleSheet, sRawDocument, null, urlStyleSheet, includeToolBar, showViewSource, showMenuIcons, editModeExclusive, sLanguage, sCountry, base64, debugMode, true, multiBar, (multiBar ? EkitCore.TOOLBAR_DEFAULT_MULTI : EkitCore.TOOLBAR_DEFAULT_SINGLE), enterBreak);
		}
		else
		{
			ekitCore = new EkitCore(caller, false, sDocument, sStyleSheet, sRawDocument, null, urlStyleSheet, includeToolBar, showViewSource, showMenuIcons, editModeExclusive, sLanguage, sCountry, base64, debugMode, false, multiBar, (multiBar ? EkitCore.TOOLBAR_DEFAULT_MULTI : EkitCore.TOOLBAR_DEFAULT_SINGLE), false, enterBreak); //LM//
		}
		...
		this.updateTitle();
		this.pack();
		//LM//this.setVisible(true);
	}

	public Ekit()
	{
		this(null, null, null, null, null, true, false, true, true, null, null, false, false, false, true, false); //LM//
	}

	/**
	 * Get the caller object. //LM// added
	 *
	 * @return the object
	 */
	public EkitCaller getCaller() { return caller; }

	/**
	 * Get the EkitCore object. //LM// added
	 *
	 * @return the object
	 */
	public EkitCore getEkitCore() { return ekitCore; }

    /**
     * Open the dialog. //LM// added
     *
     * @param posx the x position for the dialog
     * @param posy the y position for the dialog
     * @param text the text to include in the dialog
     */
    public void open(final int posx, final int posy, final String text) {
    ...
    /**
     * Operate to properly close the dialog. //LM// added
     */
    public void close() {
    	caller.closingDialog();
		setVisible(false);
    }

	/* WindowListener methods */
	public void windowClosing(WindowEvent we) //LM// modified
	{
		if(caller == null) {
			dispose();
			System.exit(0);
	...
	public static void main(String[] args)
	{
		...
		Ekit ekit = new Ekit(null, sDocument, sStyleSheet, sRawDocument, urlStyleSheet, includeToolBar, includeViewSource, includeMenuIcons, modeExclusive, sLang, sCtry, base64, debugOn, spellCheck, multibar, enterBreak); //LM//
		ekit.setVisible(true); //LM//
	}
