...
public class JSurface extends JComponent {
	...
	private Graphics graphics; // the actual graphics used by all private methods
	...
	public JSurface() { this(new DefaultSurfaceModel()); }
	 ...
	 * @return
	 */
	public JSurface getFocusedComponent() { return lastFocused; }

	public SurfaceModel getModel() { return model; }

	public void setModel(SurfaceModel model) {
		if(this.model != null) model.removePropertyChangeListener(surfaceChangesListener);
		if(this.model != null) model.removeChangeListener(surfaceChangesListener);
		if(model == null) model = new DefaultSurfaceModel();
		this.model = model;
		interrupted = false;
	...
	private String format(float f) { return String.format("%.3G", f); }
	...
	private void init() {
		...
		if(data_available) setValuesArray(model.getSurfaceVertex());
	 ...
	 */
	public void destroyImage() { repaint(); }
	 ...
	 *
	 * @return array of x,y, and z ranges in order of xmin, xmax, ymin, ymax, zmin, zmax
	 */
	 ...
	 *
	 * @param surfaceVertex the new vertices array
	 * @see #getValuesArray
	 */

	public void setValuesArray(SurfaceVertex[][] vertex) { this.surfaceVertex = vertex; }
	...
	public void doExportPNG(File file) throws IOException {
		...
		export(g2d);
		javax.imageio.ImageIO.write(bf, "PNG", file);
	}
	...
	public void paintComponent(Graphics g) {
		...
		if(data_available && !interrupted) {
			...
			if(is3D()) drawBoxGridsTicksLabels(g, true);
		}
		interrupted = false;
	}

	private boolean is3D() { return (plot_type == PlotType.WIREFRAME || plot_type == PlotType.SURFACE); }
	...
	private void printing(Graphics graphics) {
		...
		if(!data_available) drawBoxGridsTicksLabels(graphics, true);
	 ...
	 * @see #paint
	 */
	public void update(Graphics g) { paintComponent(g); } // do not erase, just paint
	...
	private synchronized void draw(Graphics graphics) {
		...
		Math.round(projector.get2DScaling() * 0.5);
	...
	public void setXLabel(String xLabel) { firePropertyChange("xLabel", this.xLabel, this.xLabel = xLabel); }

	public String getYLabel() { return yLabel; }

	public void setYLabel(String yLabel) { firePropertyChange("yLabel", this.yLabel, this.yLabel = yLabel); }
	...
	/**
	 * Determines whether a plane is plottable, i.e: does not have invalid surfaceVertex.
	 *
	...
	private final void plotSurface() {
		...
		try {
			zi = model.getZMin();
			zx = model.getZMax();
			if(zi >= zx) zx = zi + 1; //throw new NumberFormatException();
		} catch (NumberFormatException e) {
			System.out.println("PlotSurface: error in ranges");
			return;
		}
	...
	private final void plotDensity() {
		...
		try {
			...
			System.out.println("PlotDensity: error in ranges");
			return;
		}
	...
	private final void plotWireframe() {
		...
		try {
			...
			System.out.println("PlotWireframe: error in ranges");
			return;
		}
